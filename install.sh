...
echo "Scripts (-> $BIN):"
mkdir -p "$BIN"
for s in vpn-toggle.sh vpn-proxy.sh caffeine-toggle.sh sway-idle.sh sway-lock.sh waybar-run.sh openclaw-send wifi-compare.sh wifi-recover.sh touch-gestures.sh touch-resize.sh touch-resized.py wofi-tap-dismiss.py; do
